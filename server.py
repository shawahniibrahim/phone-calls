                ...
                if data['event'] == 'start':
                    ...
                    if energy > vad.energy_threshold:
                        ...
                        if not currently_speaking:
                            ...
                            waiting_for_response = False  # They're speaking, so we can respond after
                        ...
                        if currently_speaking:
                            current_segment.extend(pcm_chunk)  # Keep collecting for a bit
                            
                            # Check if 2.5 seconds have passed since last high energy
                            if last_high_energy_time and (current_time - last_high_energy_time) >= 2.5:
                                print(f"[SILENCE DETECTED] 2.5 seconds of silence/noise detected")
                                currently_speaking = False
                                
                                # Process the speech segment only if we're not waiting for a response
                                if len(current_segment) > 16000 and not waiting_for_response:  # At least 1 second
                                    print(f"\n[CLINIC AI] Transcribing {len(current_segment)} bytes...")
                                    clinic_text = await llm_client.transcribe_audio(bytes(current_segment))
                                    print(f"[CLINIC AI] Said: {clinic_text}")
                                    
                                    # Check if conversation is ending
                                    if any(word in clinic_text.lower() for word in ["goodbye", "bye", "have a great day", "take care"]):
                                    ...
                                    await send_audio_to_twilio(websocket, response_audio, stream_sid)
                                    ...
                                    print("[LISTENING] Waiting for clinic AI response...")
                                    conversation_turn += 1
                                elif waiting_for_response:
                                    print(f"[SKIPPED] Not responding - waiting for clinic AI to speak first")
                    ...
                    if audio_chunks_received % 100 == 0:
                        ...
                        print(f"[DEBUG] Chunks: {audio_chunks_received}, Energy: {energy}, Status: {status}")
